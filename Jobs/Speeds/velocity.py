...
DATA_DIR = os.path.join(HOME_DIR, 'midas-applied-ds', 'Data', 'Processed', 'ICE_trips')

files = glob.glob(os.path.join(DATA_DIR, 'ICE*'))

# all_trips = pd.read_csv(os.path.join(DATA_DIR, 'alltrips.csv'))

all_trips = pd.read_csv(os.path.join(HOME_DIR, 'NVFEL498', 'alltrips.csv'))
...
all_trips.to_csv(os.path.join(HOME_DIR, 'NVFEL498', 'alltrips_vel.csv'), index=False)