...
df = pd.read_csv('../../alltrips_final2.csv')
print(df)


pke = df['Aggressiveness']
pf = df['Normalized PKE 2']
...
plt.xticks(np.arange(0, max_x, round(max_x / 10, 2)))
plt.yticks((np.arange(0, max_y, max_y / 10)))
plt.xlabel('PKE Score (unitless)')
plt.ylabel('Fuel Rate[gpm]')
plt.title('Preliminary Regression on Normalized PKE Score')

plt.savefig('pke_normreg1.png')
